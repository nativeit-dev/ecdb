...
# Header.
cat <<-EOH
# This file is generated via https://github.com/nativeit-dev/ecdb/blob/$(fileCommit "$self")/$self
Maintainers: S. Davis <sdavis@nativeit.net> (@nativeit),
GitRepo: https://github.com/nativeit-dev/ecdb.git
EOH
...
latest="$(curl -fsSL 'https://host.nativeit.net/ecdb/version.json' | jq -r '.version')"

for variant in apache fpm fpm-alpine; do
	commit="$(dockerfileCommit "$variant")"
	fullversion="$(git show "$commit":"docker/$variant/Dockerfile" | awk '$1 == "ENV" && $2 == "VERSION" { print $3; exit }')"

...
def do_login(br, url, username, password, server):
    ...
    br['ECDB_username'] = username
    br['ECDB_password'] = password
    if server is not None:
        br['ECDB_servername'] = server
...
def test_ecdb_secrets():
    docker_secret('MYSQL_PASSWORD')
    docker_secret('MYSQL_ROOT_PASSWORD')
    docker_secret('ECDB_USER')
    docker_secret('ECDB_PASSWORD')
    docker_secret('ECDB_HOSTS')
    docker_secret('ECDB_HOST')
    docker_secret('ECDB_CONTROLHOST')
    docker_secret('ECDB_CONTROLUSER')
    docker_secret('ECDB_CONTROLPASS')
...
def test_import_from_folder(url, username, password, server, sqlfile):
    upload_dir = os.environ.get('ECDB_UPLOADDIR');
    if not upload_dir:
        pytest.skip("Missing ECDB_UPLOADDIR ENV", allow_module_level=True)
...
def test_export_to_folder(url, username, password, server, sqlfile):
    save_dir = os.environ.get('ECDB_SAVEDIR');
    if not save_dir:
        pytest.skip("Missing ECDB_SAVEDIR ENV", allow_module_level=True)
    ...
    assert(b'Dump has been saved to file /etc/ecdb/exports/db_server.sql' in response)
